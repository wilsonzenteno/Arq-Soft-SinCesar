...
(function(){
  ...
  async function loadCourse(){
    try {
      const mine = await api('/speaker.courses.mine');
      ...
      if (row) {
        ...
        setTxt('#d-title', '(curso)');
      }
    } catch (e) {
      console.warn('speaker.courses.mine falló:', e?.message || e);
    }

    try{
      const st = await api('/speaker.course.stats', { id });
      ...
      console.warn('speaker.course.stats falló:', e?.message || e);
    }
    ...
    try{
      ...
      console.warn('Fallback votos supabase (course) falló:', e?.message || e);
    }
  ...
  async function supaCourseEvals(){
    const supa = getSupa(); if (!supa) return [];
    const { data, error } = await supa
  ...
  // ===== API listas =====
  async function loadRegsViaApi(){ return await api('/speaker.course.registrations', { id }); }
  async function loadEvalsViaApi(){ return await api('/speaker.course.evaluations',   { id }); }
    ...
    btnRegs?.addEventListener('click', async ()=>{
      try{
        ...
        if (!arr.length && (gStats.registered ?? 0) > 0) {
          try{
            const rows = await supaCourseRegs();
    ...
    btnEvals?.addEventListener('click', async ()=>{
      try{
        ...
        if (!arr.length && (gStats.evalsCount ?? 0) > 0) {
          try{
            const rows = await supaCourseEvals();
