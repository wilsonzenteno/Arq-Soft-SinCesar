<?php
// server/app/Models/Supabase.php
...
class Supabase {
  ...
  public function userIdFromJwt(?string $hdr): ?string {
    if (!$hdr) return null;
    if (preg_match('/^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/', $hdr)) {
      $jwt = $hdr;
    } elseif (preg_match('/Bearer\s+(.*)$/i', $hdr, $m)) {
      $jwt = $m[1];
    } else {
      $jwt = $hdr;
    }
  ...
  /** Llamada a funciones RPC de PostgREST /rest/v1/rpc/<fn> */
  public function restRpc(string $fn, array $args = [], bool $useAnon = false): array {
    $endpoint = $this->url . "/rest/v1/rpc/" . rawurlencode($fn);
    $key = $useAnon ? $this->anon : $this->service;
    return $this->http('POST', $endpoint, [
      "apikey: $key",
      "Authorization: Bearer $key",
